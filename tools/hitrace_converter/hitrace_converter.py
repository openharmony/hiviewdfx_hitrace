...
TRACE_REGEX = "\s*(\d+)\s+(.*?)\|\d+\|[SFC]\s+:(.*?)\s+:(.*?)\s+(.*?)\s+\]\d+\[\s+\)(\d+)\s*\(\s+(\d+?)-(.*?)\s+"
text_file = ""
binary_file = ""
out_file = ""
...
def parse_text_trace_file():
	...
	as infile, open(out_file, "w+", encoding="utf-8") as outfile:
		for line in infile:
			reverse_line = line[::-1]
			trace_match = pattern.match(reverse_line)
			if trace_match:
				line = line.rstrip(' ')
				pos = line.rfind(' ')
...
def generate_one_event_str(data, cpu_id, time_stamp, one_event):
	...
	if pid == 0:
		event_str += "<idle>"
	elif cmd_lines.get(pid, "") != "":
		event_str += cmd_lines[pid]
	else:
		event_str += "<...>"
	...
	if tgids.get(pid, "") != "":
		event_str += "(" + tgids[pid].rjust(TGID_STR_MAX) + ")"
	else:
		event_str += "(-----)"
...
def parse_cpu_raw_one_page(data, result):
	...
	while cpu_raw_read_pos < end_pos:
		...
		time_stamp = page_header["time_stamp"] + event_header["time_stamp_offset"]
		event_id = struct.unpack('H', data[cpu_raw_read_pos:cpu_raw_read_pos + INT16_DATA_READ_LEN])[0]

		one_event_data = data[cpu_raw_read_pos:cpu_raw_read_pos + event_header["size"]]
		one_event_result = parse_one_event(one_event_data, event_id, page_header["core_id"], time_stamp)
		if one_event_result != "":
			result.append([time_stamp, one_event_result])

		evt_size = ((event_header["size"] + RMQ_ENTRY_ALIGN_MASK) & (~RMQ_ENTRY_ALIGN_MASK))
		cpu_raw_read_pos += evt_size
	cpu_raw_read_pos = end_pos

def parse_cpu_raw(data, len, result):
	global cpu_raw_read_pos
	cpu_raw_read_pos = 0

	while cpu_raw_read_pos < len:
...
def parse_trace_base_data(infile, file_size):
	while infile.tell() < file_size:
		type = struct.unpack('L', infile.read(INT32_DATA_READ_LEN))[0]
		len = struct.unpack('L', infile.read(INT32_DATA_READ_LEN))[0]
		data = infile.read(len)
		if type == CONTENT_TYPE_HEADER_PAGE:
			parse_header_page(data)
		elif type == CONTENT_TYPE_CMDLINES:
			parse_cmdlines(data)
		elif type == CONTENT_TYPE_TGIDS:
			parse_tgids(data)
		elif type == CONTENT_TYPE_EVENTS_FORMAT:
			parse_events_format(data)
		elif type == CONTENT_TYPE_PRINTK_FORMATS:
			parse_printk_formats(data)
		elif type == CONTENT_TYPE_KALLSYMS:
			parse_kallsyms(data)
...
def parse_trace_events_data(infile, file_size, cpu_nums, result):
	while infile.tell() < file_size:
		...
		if type >= CONTENT_TYPE_CPU_RAW and type < CONTENT_TYPE_CPU_RAW + cpu_nums:
			parse_cpu_raw(data, len, result)

trace_header = {}
def parse_binary_trace_file():
	infile = open(binary_file, 'rb')
	outfile = open(out_file, 'w')
	trace_header = parse_trace_header(infile)
	cpu_nums = (trace_header["reserved"] >> 1) & 0xf
