...
def parse(event_print_fmt, data, one_event):
	result = ""
	if print_fmt_func_map.get(event_print_fmt, "") != "":
		result = print_fmt_func_map[event_print_fmt](data, one_event)
	return result
...
def parse_cpu_frequency_limits_hm(data, one_event):
	min = parse_int_field(one_event, "min", False)
	max = parse_int_field(one_event, "max", False)
	cpu_id = parse_int_field(one_event, "cpu_id", False)

	return "min=%d max=%d cpu_id=%d" % (min, max, cpu_id)
...
def parse_ext4_da_write_end(data, one_event):
	...
	% (dev >> 20, dev & 0xfffff, ino, pos, len, copied)
...
def parse_block_rq_issue_hm(data, one_event):
	...
	sector = parse_int_field(one_event, "sector", False)
	nr_sector = parse_int_field(one_event, "nr_sector", False)
	bytes = parse_int_field(one_event, "bytes", False)
	rwbs = parse_bytes_to_str(one_event["fields"]["rwbs[8]"])
	comm = parse_bytes_to_str(one_event["fields"]["comm[16]"])
	cmd = parse_bytes_to_str(one_event["fields"]["cmd[16]"])

	return "%d,%d %s %d (%s) %d + %d [%s]" \
	% (dev >> 20, dev & 0xfffff, rwbs, bytes, cmd, sector, nr_sector, comm)
...
def parse_block_rq_issue_or_insert(data, one_event):
	...
	return "%d,%d %s %d (%s) %d + %d [%s]" \
	% (dev >> 20, dev & 0xfffff, rwbs, bytes, parse_bytes_to_str(data[cmd_pos:]), sector, nr_sector, comm)
...
def parse_ufshcd_command_hm(data, one_event):
	...
	str = parse_bytes_to_str(one_event["fields"]["str[16]"])
	...
	% (str, dev_name, tag, doorbell, transfer_len, intr, lba, opcode)
...
def parse_i2c_read(data, one_event):
	...
	return "i2c-%d #%d a=%03x f=%04x l=%d" % (adapter_nr, msg_nr, addr, flags, len)
...
def parse_i2c_write_or_reply(data, one_event):
	...
	return ("i2c-%d #%d a=%03x f=%04x l=%d " % (adapter_nr, msg_nr, addr, flags, len)) \
	+ "{:{width}d}".format(int(parse_bytes_to_str(data[buf_pos:])), width=len)
...
def parse_smbus_write_or_reply(data, one_event):
	...
	% (adapter_nr, addr, flags, command, protocol_map.get(protocol, ''), len)) + "{:{width}d}".format(int(buf), width=len)
...
def parse_regulator_set_voltage(data, one_event):
	name_pos = parse_int_field(one_event, "name", False) & 0xffff
	min = parse_int_field(one_event, "min", True)
	max = parse_int_field(one_event, "max", True)

	return "name=%s (%d-%d)" % (parse_bytes_to_str(data[name_pos:]), min, max)
