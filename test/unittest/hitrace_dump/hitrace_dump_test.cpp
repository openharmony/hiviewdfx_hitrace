...
static std::vector<std::string> GetRecordTrace()
{
    ...
    if (errorCode!= TraceErrorCode::SUCCESS) {
        GTEST_LOG_(INFO) << "errorCode: " << static_cast<int>(errorCode);
        return result;
    }
    ...
    if (ret.errorCode != TraceErrorCode::SUCCESS) {
        GTEST_LOG_(INFO) << "errorCode: " << static_cast<int>(ret.errorCode);
        return result;
    }
    CloseTrace();
...
static bool CheckBaseInfo(const std::string filePath)
{
    ...
    lseek(fd, sizeof(TraceFileHeader), SEEK_SET);

    TraceFileContentHeader contentHeader;
    read(fd, &contentHeader, sizeof(TraceFileContentHeader));
    std::string baseInfo(contentHeader.length, '\0');
    read(fd, &baseInfo[0], contentHeader.length);
    close(fd);
